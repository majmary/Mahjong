...
function candidatesForTile(tile, pool, localNumMap, localSuitMap, localKLengthMap, localRMap, group) {
    ...
    // If this group has no suitVar it is not part of the suit-locking system,
    // so no suits should be excluded from it. Only restrict when we have a suitVar.
    const lockedSuits = currentSuitVar
        ? Object.entries(localSuitMap)
...
function fillGroup(group, handPool, numVarMap, suitVarMap, kLengthMap, rVarMap, kValueSets, numVarRanges = {}) {
    const results = [];
    ...
    if (group.tiles.includes('K')) {
        ...
        function assignTile(i, matchedSoFar, poolSoFar, localNumMap, localSuitMap, localKLenMap, localRMap, localKValueSets, groupNumSuit, groupDragonSuit) {
            ...
            for (let j = 0; j < poolSoFar.length; j++) {
                ...
                // If this candidate would be the first to lock the numVar, validate that the
                // resulting base value keeps ALL groups sharing this numVar in range [1..9].
                // e.g. if numVar '1' has maxOffset +2 and cand is '8C', base=8, 8+2=10 > 9 → skip.
                if (['X', 'V', 'Y', 'M'].includes(tile) && group.numVar && !localNumMap[group.numVar]) {
                    const baseVal = parseInt(cand[0]) - (group.offset || 0);
                    const range = numVarRanges[group.numVar];
                    if (range && (baseVal + range.maxOffset > 9 || baseVal + range.minOffset < 1)) continue;
                }
...
function evaluatePattern(hand, pattern) {
    ...
    // Pre-compute valid base-value ranges for each numVar across all groups that share it.
    // Example: pattern "XX-1a XX-1a(+1) XXXX-1b(+2)" has numVar 1 with offsets 0, +1, +2.
    // Max offset is 2, so the base value of X can be at most 7 (7+2=9). This prevents
    // the engine from trying X=8 and producing a non-existent "10C" tile.
    const numVarRanges = {};
    groups.forEach(g => {
        if (!g.numVar) return;
        const off = g.offset || 0;
        if (!numVarRanges[g.numVar]) {
            numVarRanges[g.numVar] = { minOffset: off, maxOffset: off };
        } else {
            numVarRanges[g.numVar].minOffset = Math.min(numVarRanges[g.numVar].minOffset, off);
            numVarRanges[g.numVar].maxOffset = Math.max(numVarRanges[g.numVar].maxOffset, off);
        }
    });
        ...
        states.forEach(state => {
            ...
            pGroups.forEach(g => {
                ...
                partialStates.forEach(ps => {
                    const fills = fillGroup(g, ps.handPool, ps.numVarMap, ps.suitVarMap, ps.kLengthMap, ps.rVarMap, ps.kValueSets, numVarRanges);
                    fills.forEach(f => {
